# Garden Arena Reference-Matched Placement · Three.js v18
...
고정 공개 프리뷰: <https://naming-sense.github.io/lumina-garden-arena-preview/demo/?quality=mobile&v=18>
...
v18은 승인된 포탈 아치·랜턴 타워·에메랄드 나무·핑크 나무·야자수를 추가로 Blender 저폴리 버전으로 교체했다. 13개 GLB와 91개 인스턴스 기준 누적량은 2,359,679에서 550,597 rendered triangles로 내려간다. 나머지 고품질 원본은 보존했으며, 모바일 게임 투입 전에는 다음 단계로 나무/포탈 LOD, 단순 충돌체, 수로·폭포 전용 애니메이션 셰이더를 적용한다.

고유 소스 메시 자체는 합계 148,558 tris이며 같은 모델을 91번 배치해 550,597 tris가 된다. 이 수치는 바닥·글로우 효과·접지 그림자·그림자 패스는 제외한 GLB 메시 인스턴스 합계라 실제 GPU 작업량은 더 클 수 있다.
...
v18은 승인된 추가 후보를 같은 방식으로 반영했다. 포탈 아치는 22,630→5,657, 랜턴 타워는 21,258→5,314, 핑크 나무는 31,145→8,409, 야자수는 26,330→8,425 tris다. 에메랄드 나무는 자동 감면이 잎 사이를 찢는 문제를 보여 R1을 폐기하고, Blender에서 보이는 줄기·뿌리와 닫힌 저폴리 캐노피만 남긴 R5(50,158→2,213 tris)를 사용한다. 다섯 원래 웹 GLB는 `models-web/source-pre-retopo-r5/`에 보존했다. Tripo 재생성은 사용하지 않았다.
